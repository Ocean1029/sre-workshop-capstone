...
1. 打開 [`.github/workflows/cd.yml`](.github/workflows/cd.yml)，參考 CI/CD 工作坊 ch04 的做法，把 `TODO` 幾個 step 補完（self-hosted runner + `docker pull` / `docker run`）。
2. Commit 並 push：
   ```bash
...
4. CD 也綠了之後，打已部署的 `/crash`：
   ```bash
   curl <社辦機器>:8080/crash
   curl <社辦機器>:8080/crash
   ```
5. 約一分鐘內到 Discord 告警頻道確認，會看到 `AppCrashing`。
